...
const migrationAdvisoryLockID int64 = 7_431_984_201
...
func newRunner(
	...
	loadedMigrations, err := Load(files)
	if err != nil {
		return nil, err
	}
...
func (r *Runner) Down(ctx context.Context, steps int) error {
	...
	err := r.inLockedTransaction(ctx, func(tx pgx.Tx) error {
		...
		migrationsByVersion := make(map[int64]Migration, len(r.migrations))
		for _, migration := range r.migrations {
		...
		for _, appliedMigration := range applied[:steps] {
			migration := migrationsByVersion[appliedMigration.Version]
